...
| **Prosuloto antes e pós chaves** | ProSoluto dividido em Antes e Pós chaves (entrega das chaves). Ver regras de negócio abaixo. |

---

## Regras de negócio – Prosuloto antes e pós chaves

Tabela: **Prosuloto antes e pós chaves** (fonte: `administracao.prosoluto_antes_e_pos_chaves` no MotherDuck).

| # | Regra | Descrição |
|---|-------|-----------|
| 1 | **Parcelas ProSoluto** | Considera apenas parcelas com `Tipo_Baixa` em branco (não baixadas) e `Tipo_Condicao` IN ('12','PM','AT','PB','PA','PI','PQ','PS'). |
| 2 | **Vinculação** | Join id2=id1: `Cod_Centro_Custo` & `Unidade` = `codigointerno_empreendimento` & `unidade` com `cv_vendas` onde `tipovenda` = 'Venda Financiamento'. |
| 3 | **Denominador** | `valor_venda_financiamento` = SUM(`valor_contrato`) de `cv_vendas` com tipovenda = 'Venda Financiamento'. |
| 4 | **Corte antes/pós** | Tabela `planilhas.data_entrega_empreendimentos_prosoluto_antes_pos_chaves` (id_empreendimento, data_fim_obra). **Antes chaves:** Data_Vencimento ≤ data_fim_obra. **Pós chaves:** Data_Vencimento > data_fim_obra. |
| 5 | **Nomes** | Nome do empreendimento vem de `dim_empreendimentos_dinamica` quando disponível; senão, de `data_entrega_empreendimentos_prosoluto_antes_pos_chaves`. |
| 6 | **Geral Prati** | Linha consolidada (id_empreendimento nulo) com total dos empreendimentos que possuem data de corte. |

**Colunas:** id_empreendimento, nome_empreendimento, data_fim_obra, periodo (Antes chaves / Pós chaves), valor_prosoluto, valor_venda_financiamento, pct_prosoluto.
...
| **Destino** | Usada por **% Delta Ano Anterior Lucro Geral**. |
